...
shinyUI(
  navbarPage(
    ...
    tabPanel(title = "Introduction",
             ...
             column(width = 6,
                    p("In het Modoma (MOrphological Descriptors of Ornamentals through MAchine learning, 2021-2023) project hebben Floricode, Naktuinbouw en Biometris/WUR de mogelijkheden onderzocht van de toepasing van AI voor identificatie van bloemkenmerken in rozen en gerbera's. Deze bloemkenmerken zijn van belang voor zowel veredelaars als consumenten en geven een ondubbelzinnige en volledige omschrijving van de bloemvariëteiten. Daarnaast spelen deze kenmerken een belangrijke ron in distinctness, uniformity and stability (DUS) testen. Automatisering van dit process, zelfs ten dele, heeft vele voordelen, zoals objectiviteit en stabiliteit en zorgt ervoor dat het proces grotendeels gedecentraliseerd kan worden."),
                    p("Zogauw een database met bloemkenmerken beschikbaar is, kan deze op verschillende manieren worden gebruikt. Deze app toont een aantal voorbeelden gebruik makend van de gerbera data van Floricode. In de volgende tabs kun je de data bekijken, bloemen in de database zoeken die het meest lijken op een specifieke bloem, gekozen door de gebruiker, of bloemen in de database zoeken die het dichtst bij een set van waarden voor kenmerken, gekozen door de gebruiker, liggen. De laatste tab geeft een volledig overzicht van alle bloemen in de database, geordend in twee dimensies. De ordening is gebaseerd op een techniek genaamd Principal Coordinate Analysis, die probeert de berekende afstanden tussen de bloemen (gebaseerd op de bloemkenmerken) in twee dimensies weer te geven."),
                    HTML("Als je meer wilt weten over de app of het Modoma project, neem dan contact op met ", '<a href="mailto:ron.wehrens@wur.nl">Ron Wehrens</a>')
             )
    ),
    tabPanel(title = "Data",
             fluidRow(
